  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <DateSelector dateSelector={this.dateSelector} handleReset={this.handleReset} /><br /><br />
              ...
              //   :
                ...
                this.state.data.map((row, index) => (
                  ...
                  <TableRow key={index}>
                    <TableCell>
                      {
                        this.state.isLoading ? <Skeleton width={50} animation="wave" /> :
                          ++index}
                    </TableCell>
                    <TableCell>{
                      this.state.isLoading ? <Skeleton width={150} animation="wave" /> :
                        row.customerName}</TableCell>
                    <TableCell>
                      {
                        this.state.isLoading ? <Skeleton width={200} animation="wave" /> :
                    ...
                    <TableCell>{
                      this.state.isLoading ? <Skeleton width={50} animation="wave" /> :
                        row.totalRate}</TableCell>
                    <TableCell>{
                      this.state.isLoading ? <Skeleton width={50} animation="wave" /> :
                        row.totalGst}</TableCell>
                    <TableCell>{
                      this.state.isLoading ? <Skeleton width={50} animation="wave" /> :
                        row.grandTotal}</TableCell>
                    <TableCell>{
                      this.state.isLoading ? <Skeleton width={50} animation="wave" /> :
                        row.date === null ? '' : row.date.split('T')[0].split('-')[2] + '-' + row.date.split('T')[0].split('-')[1] + '-' + row.date.split('T')[0].split('-')[0]}</TableCell>
                    <TableCell>{
                      this.state.isLoading ? <Skeleton width={50} animation="wave" /> :
                        row.invoiceNo}</TableCell>
                    <TableCell align="center">{
                      this.state.isLoading ? <Skeleton width={50} animation="wave" /> :
                        row.expense !== 0 ? <CheckCircleIcon /> :
                    ...
                    <TableCell align="center"><IconButton size='small'>
                      {this.state.isLoading ? <Skeleton width={50} animation="wave" /> :
                      <GetAppIcon onClick={() => handleDownloadPdf(row)} />}</IconButton>
                    </TableCell>
                  </TableRow>
                ))}
