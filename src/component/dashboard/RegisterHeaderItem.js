...
import Logout from "../common/LogoutM";
    ...
    <div>
      ...
      >
        <Typography style={{ paddingLeft: "15px" }}>
          <strong>{JSON.parse(localStorage.getItem("token")).name}</strong>
        </Typography>
        <MenuItem
          onClick={() => {
      ...
      <Suspense fallback={<div />}>
        <ItemForm
          open={itemFormOpen}
          closeItemForm={() => setItemFormOpen(false)}
        />
        <VendorForm
          open={vendorFormOpen}
          closeItemForm={() => setVendorFormOpen(false)}
        />
        <CustomerForm
          open={customerFormOpen}
          closeItemForm={() => setCustomerFormOpen(false)}
        />
        <StockForm
          open={stockFormOpen}
          closeItemForm={() => setStockFormOpen(false)}
        />
        <ExpenseForm
          open={expenseFormOpen}
          closeItemForm={() => setExpenseFormOpen(false)}
        />
      </Suspense>
    </div>
  );
