...
export default function App() {
  ...
  return ( 
    <div className = 'App' >
		<Switch>
			<Route exact path = "/" ><Suspense fallback = {<div />}><FormLogin /></Suspense></Route> 
			<Route path = "/dashboard" > {matches ? < Dashboard /> : < DashboardM />} </Route> 
			<Route exact path = "/stock" > { matches ? < Stock /> : < StockM />} </Route> 
			<Route exact path = "/sales" > {matches ? < Sales /> : < SalesM />}</Route> 
			<Route exact path = "/vendors" ><Vendors /></Route> 
			<Route exact path = "/customers" ><Customer /></Route> 
			<Route exact path = "/item_master" ><ItemMaster /></Route> 
			<Route exact path = "/purchase" > {  matches ? < Expenses /> : < ExpensesM />}</Route> 
			<Route path = "*" ><div> Invalid route </div> </Route>
		</Switch> 
    </div>
  );
}